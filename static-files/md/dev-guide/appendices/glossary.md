...
the context of a host application which gives them the environment they need.
Firefox and Thuunderbird are the most obvious hosts for Mozilla add-ons, but
at present only Firefix is supported as a host for add-ons developed using the
Add-on SDK.</span>
