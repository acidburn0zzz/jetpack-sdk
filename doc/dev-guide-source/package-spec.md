...
<tr>
  <td id="content-permissions"><code>content-permissions</code></td>
  <td><p>String, or array of strings, each of which is the name of a domain whose content will be accessible
  to content scripts <em>in addition to</em> the page to which the content scripts are attached.</p>
  <p>Content scripts will be allowed to make XMLHttpRequests
  to these domains. Also, if the pages to which the content scripts are attach embed iframes, and these iframes
  contain web content served from one of these "whitelisted" domains, then content scripts will be able to access
  that content.</p>
  <p>See <a href="dev-guide/guides/content-scripts/cross-domain.html">Cross-domain Content Scripts</a>.</p></td>
</tr>
